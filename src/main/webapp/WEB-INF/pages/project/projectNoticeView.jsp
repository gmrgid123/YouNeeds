	...
	<div style="margin-top: 50px; margin-bottom: 50px; padding-right: 20%; padding-left: 20%;">
		<div class="content_body">
			<div class="content_title">
				<h1 style="text-align: center;">${dto.p_notice_title}</h1>
				<div style="text-align: right;">
					<span class="regdate">작성일 : <fmt:formatDate value="${dto.p_notice_regdate}" pattern="yyyy-MM-dd  HH:mm:ss"/></span>
				</div>
			</div>
			...
			<div class="content" style="margin-right: 10%; margin-left: 10%;">
				<p>${dto.p_notice_content}</p>
			</div>
		</div>
		
		<c:if test="${sessionScope.member.m_uid eq dto.projectDto.creator_uid }">
			<div style="float: right;">
				<input type="button" class="btn btn-secondary updateBtn" value="수정" onclick="location.href='';">
				<input type="button" class="btn btn-secondary updateBtn" value="삭제" onclick="delcheck();"/>
			</div>
		</c:if>
		
		<div class="btn_div">
			<input id="listGo" type="button" class="btn btn-secondary"  value="목록으로 돌아가기"  onclick="location.href='pjNoticeList?p_id=${dto.p_id}&page=1'"/>
		</div>
